...
describe("StateService API Tests", () => {
  ...
  describe("POST /login", () => {
    it("should set login state to true", async () => {
      ...
      const findOneStub = sinon
        .stub(State, "findOne")
        .resolves({ login_state: false, save: sinon.stub() });
  ...
  describe("PUT /system-state", () => {
    it("should update the system state", async () => {
      const findOneStub = sinon
        .stub(State, "findOne")
        .resolves({ system_state: "INIT", save: sinon.stub() });
    ...
    it("should return 400 for invalid system state", async () => {
      ...
      expect(res.body).to.have.property("message", "Invalid system state.");
    });
